...
def dirichlet_allocation_balanced(trainset, alpha, num_classes, num_clients):
    """
    ...
    images_per_client = len(trainset.targets) // num_clients
    ...
    normalized_proportions = dirichlet_proportions / np.sum(dirichlet_proportions, axis=0, keepdims=True)
...
def create_dir_data(trainset,alpha,num_classes,num_clients,batch_size,seed):
    # Apply Dirichlet allocation
    client_data_indices = dirichlet_allocation_balanced(trainset,alpha,num_classes,num_clients)
    clients_data = {i: Subset(trainset, client_data_indices[i]) for i in range(num_clients)}
